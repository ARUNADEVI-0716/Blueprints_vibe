...
//             if (session.user.email !== import.meta.env.VITE_OFFICER_EMAIL) {
    ...
    return (
        <div className="page-wrapper page-enter">
            ...
            <nav className="bg-white border-b border-purple-100 sticky top-0 z-10">
                <div className="max-w-7xl mx-auto px-10 flex items-center justify-between" style={{ height: '72px' }}>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <button onClick={handleLogout}
                                className="flex items-center gap-2 text-base text-gray-400 hover:text-purple-600 font-semibold transition-all bg-white border border-purple-100 rounded-full px-5 py-2.5 hover:border-purple-300 active:scale-95">
                            <svg width="18" height="18" viewBox="0 0 15 15" fill="none">
                                <path d="M9 2H13V13H9M6 10L3 7.5M3 7.5L6 5M3 7.5H10" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/>
                            </svg>
                            Sign out
            ...
            <main className="max-w-7xl mx-auto px-10 py-14">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                    {[
                        { label: 'Total',    value: totalCount,    color: 'bg-purple-100 text-purple-600',  icon: '📋', filter: 'all' },
                        { label: 'Pending',  value: pendingCount,  color: 'bg-yellow-100 text-yellow-600',  icon: '⏳', filter: 'pending' },
                        { label: 'Approved', value: approvedCount, color: 'bg-emerald-100 text-emerald-600', icon: '✅', filter: 'approved' },
                        { label: 'Rejected', value: rejectedCount, color: 'bg-red-100 text-red-600',        icon: '❌', filter: 'rejected' },
                    ].map((stat, i) => (
                        <div key={stat.label}
                             className={`fade-up fade-up-${i + 2} bg-white rounded-3xl p-8 border border-purple-100 hover:border-purple-300 transition-all hover:shadow-lg hover:shadow-purple-50 cursor-pointer active:scale-95`}
                ...
                {/* Search + Filter */}
                <div className="fade-up fade-up-3 flex items-center gap-4 mb-8 flex-wrap">
                    <div className="relative flex-1 min-w-64">
                        <span className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-300">
                            <svg width="20" height="20" viewBox="0 0 15 15" fill="none">
                                <circle cx="6.5" cy="6.5" r="5" stroke="currentColor" strokeWidth="1.2"/>
                    ...
                    <div className="flex items-center gap-2">
                        {(['all', 'pending', 'approved', 'rejected'] as const).map(f => (
                            <button key={f} onClick={() => setFilter(f)}
                                    className={`px-5 py-3 rounded-xl text-base font-semibold transition-all capitalize active:scale-95 ${
                                        filter === f
                                            ? 'bg-purple-600 text-white shadow-md shadow-purple-200'
                                            : 'bg-white text-gray-500 border border-purple-100 hover:border-purple-300'
                                    }`}>
                        ...
                        {filtered.map(app => (
                            <div key={app.id}
                                ...
                                <div className="col-span-2">
                                    <button onClick={() => navigate(`/officer/applications/${app.id}`)}
                                            className="w-full bg-purple-600 hover:bg-purple-700 text-white font-semibold text-sm px-4 py-2.5 rounded-xl transition-all active:scale-95">
                                        Review →
                                    </button>
                                </div>
                            </div>
